...
- **The space separator was never the gap, and that is why the `T` form comes too.** Temporal accepts a space wherever it accepts `T`, so `day('2026-08-08 12:00:00Z')`, the offset form and the `[Zone]` form all answered before this change — verified against master. Exactly one hole existed: a clock naming NO zone, and both separators fell in it. Refusing one spelling would have made the rule about punctuation instead of about information.

### Changed

- **`day()` throws a `TypeError` when `tz` meets a zoneless clock, instead of answering.** The rule it protects is measurable, not stylistic. On 2026-08-24 SQLite's `datetime('now')` was `2026-08-24 01:57:31` and `datetime('now','localtime')` was `2026-08-23 21:57:31` — **different days** — because `datetime()` defaults to UTC. Naming a zone means convert, and a zoneless clock gives nothing to convert from, so the caller who stores the default and asks for a local day is exactly the caller a silent answer would mislead. Put the zone in the string (`Z`, an offset, or `[Zone]`) and it converts as before. Only that pair is refused: `day('2026-08-08', 'Asia/Tokyo')` still answers `'2026-08-08'`, unchanged, because no information is discarded there. It is the sibling of the existing `two time zones` refusal — both fire when the arguments ask for something the inputs cannot support.
- **The hour stops at 23, and that is the rule rather than a bound: daymath drops a clock only when the clock cannot change the date.** ISO `24:00` is midnight starting the NEXT day, so dropping it would answer the day before. SQLite accepts `24:00:00` and echoes it back unchanged; Temporal refuses it; daymath refuses it, so no stored value can produce an off-by-one day. Seconds stop at 59 for a matching reason: `23:59:60` is a value SQLite refuses to store and Temporal silently clamps to `23:59:59`, so shape settles it here rather than letting either decide.
...
- **The change is confined to exactly two inputs, proved by an A/B in one process rather than argued.** The battery ran master's `index.js` and this one over the identical 62,928-probe set, and 242 rows differed. Every differing row index fell in 776-783 — the four probes each for `'2026-08-08 12:00:00'` and `'2026-08-08T12:00'`, which still sat in the JUNK list at that point. Nothing else moved, on any of the 69 exports. Doing this first mattered: the two strings deliberately go from throwing to answering, so re-recording the baseline alone would have hidden any other drift inside a wholesale hash change.
- The cross-runtime baseline is re-recorded on purpose, 62,928 calls to 66,240. `battery.mjs` gains a `WALL_CLOCKS` list probed WITH a second argument, which is what puts the new refusal under a cross-runtime hash, and three JUNK entries for the clocks that must stay refused. PASS on Node 26.7.0, Deno 2.9.5 (native Temporal) and Bun 1.3.14 (bundled).
...
- Bundle cost, measured by `npm run size`: the three-call program goes 16,295 B to 16,374 B gzip (+79 B), the whole surface 21,550 B to 21,702 B (+152 B), and the program plus `day()` 19,349 B to 19,508 B (+159 B). The baseline is re-recorded.
