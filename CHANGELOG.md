...
- **`index.d.ts` now carries `@example` blocks, and it had none at all.** Measured before starting: zero examples across 69 declarations, while `index.js` had ten and every one was inside the `day()` block. `package.json` declares exactly one types path — `"types": "./index.d.ts"`, repeated in the `exports` map — so a TypeScript editor resolves documentation from the `.d.ts` and never parses `index.js`. **Every example daymath had was invisible on hover**, which is where a typed caller reads. **All 70 declaration sites now have a hover tooltip, and 65 carry a runnable `@example`** — before this, 43 of 69 exports showed nothing but their signature. Seventy, not sixty-nine, because `day` is overloaded and an editor shows the doc of the overload it RESOLVES: the whole contract block sat on `day(tz?)`, the zone-only form, while `day(moment, tz?)` — the one that takes a `Date`, epoch milliseconds, a day string or a SQLite `DATETIME` — had none. Counting by export name hid that; counting by declaration site found it. The block now sits on the moment overload, where nearly all of it applies, and the clock-reading overload gets its own. The first pass covered only the traps, which left sibling asymmetries that are their own defect: `min` documented and `max` bare, `startOfWeek` documented and `endOfWeek` bare, `addMonths` documented and `subMonths` bare, `differenceInMonths` documented and `differenceInYears` bare on the same counting rule, `eachDayOfInterval` documented and the other two bare on the same inclusivity. Documenting one of a pair implies the other differs. `isLeapYear` was bare while carrying the library's single most dangerous trap — `isLeapYear('2567-01-01')` is `false` while Buddhist 2567 is ISO 2024 and IS a leap year, and the two rules disagree in 49 of the 101 Buddhist years from 2500 to 2600 with nothing thrown. Examples are chosen by where daymath *surprises* a date-fns user rather than by call frequency: `getMonth` is 1-12, `getDay` is 1=Monday…7=Sunday, `addMonths`/`setMonth`/`setDate`/`setYear` clamp instead of rolling, `startOfWeek` defaults to Sunday, `differenceInMonths` counts by `addMonths`, `min` takes an array, and the interval helpers are inclusive at both ends. `format` and `isValid` show what they refuse.
- **`npm run test:examples` executes every `@example` and asserts its stated answer.** **It reads `README.md` too, which is the file that most needed it**: it ships in the tarball, it is what a developer opens first, and three of its `day()` lines had silently rotted. Fixing those three closed the instances; reading the file closes the class. 112 claims asserted across `index.d.ts`, `index.js` and `README.md`, 25 correctly not assertable, and the accounting closes with 0 unaccounted. It is a new gate for a real blind spot: `tsc` type-checks the declarations and ignores the comments, the differential harness compares daymath to date-fns and never opens a JSDoc block, and the cross-runtime battery enumerates exports rather than documentation. So an example could go stale on any behaviour change with every gate still green — and the example is the line a caller copies. Proved to block by planting a wrong answer: exit 1, naming the file, line, expected and actual.
- **The gate reconciles its own count against the source, because it needed to.** A one-line `/** … @example … */` block was invisible to the collector, so **27 examples were unchecked and the run was green** — the exact failure the gate exists to prevent, inside the gate. Fixing the regex would have closed that instance and not the class, so the script now counts raw `@example` occurrences per file and fails when fewer were read than written. Planted by reverting the regex: `97 @example tags are written in the source but only 70 were read`, exit 1. A **ceiling on skips** catches the other direction, a checked claim being downgraded to prose — which the accounting cannot see, because the written total does not move. A ceiling rather than a floor on assertions, because a floor needs a bump for any ordinary addition or deletion while catching nothing a ceiling misses.

### Fixed

- **Six clock-dependent examples had silently rotted, three in `index.js` and three in `README.md`.** They claimed `day()` was `'2026-08-08'`, `day('Asia/Tokyo')` was `'2026-08-09'` and `addDays(day(), 2)` was `'2026-08-10'` — all true the day they were written and none true afterwards, because `day()` reads a clock. A literal that depends on today's date can only rot, so all six now read as prose. No behaviour changed; the documentation stopped lying.

  The `README.md` three matter more than the `index.js` three, because **`README.md` ships in the npm tarball** and `scripts/examples.mjs` cannot reach it. They were found by sweeping the class rather than the diff: the gate establishes that a clock-dependent literal must be prose, applies it to `index.js`, and argues for it here — so leaving the same three literals in the shipped README made this entry contradict itself.
...
- **A zoneless wall clock on a day is now a day, so a SQLite `DATETIME` goes straight in.** `parse('2026-08-08 12:00:00')` is `'2026-08-08'`, and so is the `T` spelling `'2026-08-08T12:00'`. The clock is dropped and never read. Measured on SQLite 3.54.0, this is every shape it emits: `datetime()` and `CURRENT_TIMESTAMP` give `YYYY-MM-DD HH:MM:SS`, and `strftime('%Y-%m-%d %H:%M:%f')` and `datetime('now','subsec')` add three fractional digits. A column value needs no reshaping before daymath reads it.
- **It lands in `bareDay`, the funnel all 69 exports share, so this is not a `day()` feature.** `addDays('2026-08-08 12:00:00', 30)` is `'2026-09-07'` and `getYear('2026-08-08T12:00')` is `2026`. `day()` still owns instants and zones, and `addDays` is still not routed through it: nothing here reads a clock, so no export became impure.
- **The separator was never the gap, which is why `T`, `t` and a space are all taken.** Temporal accepts a space and a lowercase `t` wherever it accepts `T`, so `day('2026-08-08 12:00:00Z')`, `day('2026-08-08t12:00:00Z')`, the offset form and the `[Zone]` form all answered before this change — verified against master, not assumed. Exactly one hole existed: a clock naming NO zone, and every separator fell in it. Accepting two of the three would have made the rule about punctuation instead of about information, which is the gap this change exists to close.

### Changed

- **`day()` throws a `TypeError` when `tz` meets a zoneless clock, instead of answering — and it throws AFTER both arguments are judged separately.** The guard is about the argument PAIR, so it must never pre-empt a fault in either argument alone: `day('2026-02-30 12:00:00', 'utc')` reports the impossible day, and `day('2026-08-08T12:00', 'Bad/Zone')` reports the zone, which is the same message a bare day draws. An earlier revision ran the guard before the parse and blamed the zone for a bad stored day. The rule it protects is measurable, not stylistic. Measured at 2026-08-23 21:57 in `America/New_York`: SQLite's `datetime('now')` read `2026-08-24 01:57:31` and `datetime('now','localtime')` read `2026-08-23 21:57:31` — **different days, at one instant** — because `datetime()` defaults to UTC. Naming a zone means convert, and a zoneless clock gives nothing to convert from, so the caller who stores the default and asks for a local day is exactly the caller a silent answer would mislead. Put the zone in the string (`Z`, an offset, or `[Zone]`) and it converts as before. Only that pair is refused: `day('2026-08-08', 'Asia/Tokyo')` still answers `'2026-08-08'`, unchanged, because no information is discarded there. It is the sibling of the existing `two time zones` refusal — both fire when the arguments ask for something the inputs cannot support.
- **The hour is the only bound, because the hour is the only field that can change the date.** That is the whole rule: daymath drops a clock only when the clock cannot move the day. ISO `24:00` is midnight starting the NEXT day, so dropping it would answer the day before. SQLite accepts `24:00:00` and echoes it back unchanged; Temporal refuses it; daymath refuses it, so no stored value can produce an off-by-one day. A leap second `23:59:60` and a fraction of any length both stay inside their own day, so both are accepted — and `day('2026-08-08 23:59:60Z')` already answered on the instant path, where Temporal clamps to `:59`, so refusing the zoneless twin would have been the separator gap again on a different field.
- A clock is dropped while a `[u-ca=…]` annotation still rides along, and the two are consistent rather than arbitrary: an annotation renumbers the fields daymath answers with, and a clock names a field daymath does not have. `getYear('2026-01-31 12:00:00[u-ca=buddhist]')` is `2569`.
...
- **Nothing that answered before answers differently now, and that is measured rather than argued.** An A/B ran master's `index.js` and this one in one process over master's own unchanged 62,928-probe battery, and every differing row was classified:

  | old → new | rows |
  |---|---|
  | answered → **throws** | **0** |
  | answered → answered, different value | 4 |
  | threw → answers | 148 |
  | threw → threw, same class, new message | 4,720 |
  | threw → threw, different class | 48 |

  The 4 are `isValid` going `false` to `true` on the two strings, which is the feature. The 4,720 are one edit: the day-shape refusal message now names the clock, and it appears under five argument labels. The 48 are downstream consequences with no surprise in them — a first argument that used to fail the shape check now passes, so the *next* argument's own fault surfaces instead (36 report a missing `dateRight`, 10 report a non-interval, 2 are the new zone guard). **The load-bearing number is the zero.** Doing this A/B before re-recording mattered, because a wholesale hash change would have hidden any real drift inside it.
- The cross-runtime baseline is re-recorded on purpose, 62,928 calls to 67,206. `battery.mjs` gains a `WALL_CLOCKS` list probed WITH a second argument, which is what puts the new refusal under a cross-runtime hash, plus JUNK entries for the clocks that must stay refused. PASS on Node 26.7.0, Deno 2.9.5 (native Temporal) and Bun 1.3.14 (bundled).
- The differential harness against date-fns 4.4.0 is unchanged: 52 of 57 functions match exactly, every known divergence count still equals its baseline. Coverage stays at 100% of lines, branches and functions across 97 tests.
- ReDoS: the new pattern is anchored with no nested quantifier, and it was timed rather than reasoned about. An 8 MB input that fails at the last character resolves in 4.74 ms, and doubling the input doubles the time. `ISO_DAY` and `ZONE_LIKE` were re-timed for the same reason and are still linear. `'[u-ca='.repeat(64000)` through `day()` costs 0.52 ms, so the earlier 9.0 s and 6.8 s quadratic regressions do not return. CodeQL `js/polynomial-redos` passes.
- Bundle cost, measured by `npm run size`: the three-call program goes 16,295 B to 16,432 B gzip (+137 B), the whole surface 21,550 B to 21,762 B (+212 B), and the program plus `day()` 19,349 B to 19,568 B (+219 B). The baseline is re-recorded.
